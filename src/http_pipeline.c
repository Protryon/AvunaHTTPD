...
void handle_vhost_htdocs(struct request_session* rs) {
    ...
    char* extraPath = NULL;
    int isStatic = 1;
    size_t htdl = strlen(vhost->sub.htdocs.htdocs);
    size_t pl = strlen(rs->request->path);
    char* tp = pmalloc(rs->pool, htdl + pl);
    memcpy(tp, vhost->sub.htdocs.htdocs, htdl);
    ...
    int indf = 0;
    if (!ff && !access(tp, R_OK)) { // TODO: extra paths?
    ...
    rs->response->body->stream_fd = -1;
    rs->response->body->stream_type = STREAM_TYPE_INVALID;
    if (rs->response->body != NULL && rs->response->body->mime_type != NULL) {
