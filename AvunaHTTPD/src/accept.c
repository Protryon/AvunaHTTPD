...
void run_accept(struct accept_param* param) {
	...
	while (1) {
		...
		if (poll(&spfd, 1, -1) < 0) {
			errlog(param->logsess, "Error while polling server: %s\n", strerror(errno));
			xfree(c);
			continue;
		}
		if ((spfd.revents ^ POLLIN) != 0) {
			errlog(param->logsess, "Error after polling server: %i (poll revents), closing server!\n", spfd.revents);
			xfree(c);
		...
		if (cfd < 0) {
			...
			errlog(param->logsess, "Error while accepting client: %s\n", strerror(errno));
			xfree(c);
			continue;
		}
		c->fd = cfd;
		...
		if (fcntl(cfd, F_SETFL, fcntl(cfd, F_GETFL) | O_NONBLOCK) < 0) {
			errlog(param->logsess, "Setting O_NONBLOCK failed! %s, this error cannot be recovered, closing client.\n", strerror(errno));
			close(cfd);
			continue;
		}
		...
		if (add_collection(work->conns, c)) { // TODO: send to lowest load, not random
			if (errno == EINVAL) {
				...
				errlog(param->logsess, "Collection failure! Closing client. %s\n", strerror(errno));
			}
			close(cfd);
			continue;
		}
		if (write(work->pipes[1], &onec, 1) < 1) {
			errlog(param->logsess, "Failed to write to wakeup pipe! Things may slow down. %s\n", strerror(errno));
		}
	}
}
